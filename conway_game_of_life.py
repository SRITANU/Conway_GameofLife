...
def next_generation_pattern_generator(cols, rows, binary_array, next_generation):
	"""
	This function generates and returns the next generation pattern array based on
	...
	for i in range(1,rows-1):
            for j in range(1,cols-1):
                next_generation[i][j] = count_and_process_neighbours(i, j, binary_array)

        return next_generation

def count_and_process_neighbours(a,b,current_pattern):
	"""
	This function counts the neighbours of a particular cell,decides the value of 
	the cell for the next_generation pattern and returns it to the next_generation
	_pattern_generator function.
	...
	A particular cells has eight cells in a Moore neighbourhood,as illustrated in the following
	diagram:
	...
	So,the two loops runs from (i-1) to (i+1) and (j-1) to (j+1) to determine the postion and value
	of the neighbourhood cells,considering the fact that the own value of a particular cell should 
        not be taken into account while counting and processing the neighbourhood cells.
	"""
